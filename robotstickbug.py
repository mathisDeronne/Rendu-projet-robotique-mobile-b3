...
def testIKOriented(sim, robot):
    ...
    for i in range(6):
        leg= robot.legs[i+1]
        thetas= kinematicsdanse.computeIKOriented(val, 0, 0, i+1)
        for m in range(0,3):
...
sim.setRobotPose([12, 12, 0.5], to_pybullet_quaternion(0, 0, 0, degrees=True))
...
# robot.legs[1][0] premier moteur de la premi̬re patte du robot
...
while True:
    ...
    for l in index_patte1:
        thetas = kinematicsdanse.triangle2(0, -0.05, 0, 0.08, sim.t*3, leg_id=l)
        for m in range(0,3):
            robot.legs[l][m].goal_position = thetas[m]

    for l in index_patte2:
        thetas = kinematicsdanse.triangle(0, -0.05, 0, 0.08, sim.t*3+1, leg_id= l)
        for m in range(0,3):
            robot.legs[l][m].goal_position = thetas[m]
    # fin rotation robot
